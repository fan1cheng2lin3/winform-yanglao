...
namespace 养老系统
{
    public partial class ceshi : Form
    {
        ...
        public ceshi()
        {
            ...
            SetCurrentUser(label1.Text);
            ReceiveMsg(msgPath);
            user = GetNumberFromLoginTable();
            toUser = GetNumberFromLoginTable2();
            //MessageBox.Show(GetNumberFromLoginTable());
        }
        ...
        private void ceshi_Load(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show($"Username: {currentUser}, toUsername: {toUser}");
        }
        ...
        private void DisplayVideoThumbnails()
        {
            // 假设您已经从数据库或文件系统获取了视频截图的路径
            string[] videoThumbnailPaths = GetVideoThumbnailPaths(); // 自定义方法获取视频截图路径

            for (int i = 0; i < videoThumbnailPaths.Length; i++)
            {
                PictureBox pictureBox = new PictureBox
                {
                    Image = Image.FromFile(videoThumbnailPaths[i]), // 加载视频截图
                ...
                {
                    // 点击PictureBox时显示媒体播放器并播放视频
                    ShowMediaPlayerAndPlayVideo($"path_to_video_{i}.mp4"); // 替换为实际视频文件路径
                };

                this.panel1.Controls.Add(pictureBox); // 假设flowLayoutPanel用于容纳PictureBox
            }
        }
        ...
        private void ceshi_FormClosed(object sender, FormClosedEventArgs e)
        {
            // 禁用消息接收
            shouldReceiveMsg = false;
